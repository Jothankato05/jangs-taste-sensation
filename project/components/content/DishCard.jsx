...
export function DishCard({ name, description, price, image, badge, atmosphere = "warm", style }) {
  ...
  return (
    <article
      ...
      style={{ fontFamily: "var(--font-sans)", cursor: "pointer", ...style }}
    >
      <div style={{ position: "relative", aspectRatio: "4 / 3", overflow: "hidden", borderRadius: "2px 28px 2px 2px", border: "1px solid var(--border-subtle)" }}>
        ...
        {badge && <div style={{ position: "absolute", top: 14, left: 14 }}><Badge>{badge}</Badge></div>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 16, marginTop: 16, borderTop: "1px solid var(--border-subtle)", paddingTop: 12 }}>
        <h3 style={{ margin: 0, fontSize: 14, fontWeight: "var(--weight-semibold)", letterSpacing: "0.18em", textTransform: "uppercase", color: hover ? "var(--morning-fog)" : "var(--text-primary)", transition: "color var(--duration-base)" }}>{name}</h3>
