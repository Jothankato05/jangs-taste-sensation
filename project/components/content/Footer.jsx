...
export function Footer({ columns = [], note, style }) {
  return (
        ...
        <div style={{ display: "flex", gap: 72, flexWrap: "wrap" }}>
          {columns.map((col) => (
            <div key={col.title} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {col.links.map((l) => (
                <a key={l} href="#" onClick={(e) => e.preventDefault()} style={{ fontSize: 13, fontWeight: "var(--weight-light)", color: "var(--text-secondary)", textDecoration: "none", letterSpacing: "0.06em" }}>{l}</a>
              ))}
            </div>
          ))}
        </div>
