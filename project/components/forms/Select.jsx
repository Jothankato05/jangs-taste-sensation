...
export function Select({ label, value, onChange, options = [], placeholder, style }) {
  ...
  return (
    <label style={{ display: "block", fontFamily: "var(--font-sans)", ...style }}>
      {label && <span style={{ display: "block", fontSize: 10, fontWeight: "var(--weight-medium)", letterSpacing: "0.28em", textTransform: "uppercase", color: "var(--text-secondary)", marginBottom: 10 }}>{label}</span>}
      <div style={{ position: "relative" }}>
        <select
          ...
          style={{
            ...
            padding: "10px 24px 12px 0", color: value ? "var(--text-primary)" : "var(--text-tertiary)",
            fontFamily: "var(--font-sans)", fontSize: 16, fontWeight: "var(--weight-light)", letterSpacing: "0.04em",
            transition: "border-color var(--duration-base) var(--ease-cinematic)", cursor: "pointer",
          }}
        >
          {placeholder && <option value="" disabled style={{ color: "#000" }}>{placeholder}</option>}
          {options.map((o) => (
